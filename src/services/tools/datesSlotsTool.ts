...
import {
  IDatesSlotsRepository,
  DatesSlotsRepository,
} from "../repositories/datesSlotsRepository";
...
export const datesSlotsTool = (ai: Genkit) =>
  ai.defineTool(
    {
      name: "datesSlotsTool",
      ...
      inputSchema: z.object({
        fecha: z
          .string()
          .describe(
            `Fecha en formato YYYY-MM-DD (ej. "2024-07-01"). Asegúrate de usar la fecha correcta basándote en que hoy es ${new Date().toISOString().split("T")[0]}`,
          ),
      }),
      outputSchema: z
        .array(z.string())
        .describe(
          'Un array de timestamps ISO 8601 disponibles para la fecha consultada (ej. ["2024-07-01T09:00:00Z", "2024-07-01T10:00:00Z"])',
        ),
    },
    async ({ fecha }) => {
